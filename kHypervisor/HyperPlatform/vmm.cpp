...
//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ FlagRegister *VmmpGetFlagReg(
...
//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ ProcessorData *VmmpGetProcessorData(
    GuestContext *guest_context) {
...
//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ VOID SetvCpuMode(GuestContext *guest_context,
                                        CPU_MODE CpuMode) {
...
//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ VOID VmmpSetvCpuVmx(GuestContext *guest_context,
                                           VCPUVMX *VCPUVMX) {
  guest_context->stack->processor_data->vcpu_vmx = VCPUVMX;
}

//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ EptData *VmmGetCurrentEpt02Pointer(
    GuestContext *guest_context) {
  return guest_context->stack->processor_data->EptDat02;
}
//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ VOID
VmmSaveCurrentEpt02Pointer(GuestContext *guest_context, EptData *Ept02) {
  guest_context->stack->processor_data->EptDat02 = Ept02;
}

//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ EptData *VmmGetCurrentEpt12Pointer(
    GuestContext *guest_context) {
  return guest_context->stack->processor_data->EptDat12;
}
//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ VOID
VmmSaveCurrentEpt12Pointer(GuestContext *guest_context, EptData *Ept12) {
  guest_context->stack->processor_data->EptDat12 = Ept12;
}
//----------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ EptData *VmmGetCurrentEpt01Pointer(
    GuestContext *guest_context) {
...
//-----------------------------------------------------------------------------------------------------------------------//
#pragma warning(pop)
_Use_decl_annotations_ static void VmmpHandleVmExitForL1(
    GuestContext *guest_context) {
...
//-----------------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ VmxSecondaryProcessorBasedControls
GetSecondaryCpuBasedVmexitCtrlForLevel1(_In_ GuestContext *guest_context) {
  ...
  VmcsVmRead32(VmcsField::kSecondaryVmExecControl, vmcs12_va,
               &CpuBasedVmExitCtrl);
  SecondaryCtrl = {CpuBasedVmExitCtrl};
  return SecondaryCtrl;
}
//-----------------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ PUCHAR GetMsrBitmap(_In_ GuestContext *guest_context) {
  ULONG_PTR vmcs12_va = 0;
...
//-----------------------------------------------------------------------------------------------------------------------//
_Use_decl_annotations_ VmxProcessorBasedControls
GetCpuBasedVmexitCtrlForLevel1(_In_ GuestContext *guest_context) {
  ULONG_PTR vmcs12_va = 0;
  ULONG32 CpuBasedVmExitCtrl = 0;
  VmxProcessorBasedControls ctrl = {0};
  ...
  VmcsVmRead32(VmcsField::kCpuBasedVmExecControl, vmcs12_va,
               &CpuBasedVmExitCtrl);
...
_Use_decl_annotations_ static NTSTATUS VmmpHandleExceptionForL2(
  ...
  VmExitInterruptionInformationField exception = {
  ...
  if (ExceptionBitmap & (1 << exception.fields.vector)) {
    ...
    HYPERPLATFORM_LOG_DEBUG("Exception vector: %x Rip: %p ",
                            exception.fields.vector,
                            UtilVmRead64(VmcsField::kGuestRip));
...
_Use_decl_annotations_ static NTSTATUS VmmpHandleVmcallForL2(
    _In_ VmExitInformation exit_reason, _In_ GuestContext *guest_context) {
...
_Use_decl_annotations_ static NTSTATUS VmmpHandleRdmsrForL2(
    _In_ GuestContext *guest_context) {
  VmxProcessorBasedControls ctrl =
      GetCpuBasedVmexitCtrlForLevel1(guest_context);
  PUCHAR msr_bitmap =
      (PUCHAR)UtilVaFromPa((ULONG64)GetMsrBitmap(guest_context));
...
_Use_decl_annotations_ static NTSTATUS VmmpHandleDescriptorTableAccessForL2(
    _In_ GuestContext *guest_context) {
  VmxSecondaryProcessorBasedControls ctrl =
      GetSecondaryCpuBasedVmexitCtrlForLevel1(guest_context);
...
_Use_decl_annotations_ static NTSTATUS VmmpHandleEptViolationForL2(
  ...
  /*
     ...
     |	EPT0-1 PML4 -> PDPTR -> PDE -> PTE		| L0 |	  HPA
     <-------	<---------------------------------------
                  -------------------
  */
  NTSTATUS status = STATUS_UNSUCCESSFUL;
  do {
    ...
    if (!exit_qualification.fields.ept_readable &&
      ...
      EptCommonEntry *Ept12Pte = EptGetEptPtEntry(
          guest_context->stack->processor_data->EptDat12, fault_pa);
      ...
      NT_ASSERT((VOID *)UtilPaFromPfn(Ept12Pte->fields.physial_address) ==
                PAGE_ALIGN(fault_pa));

      // Translate L1 GPA to HPA
      EptCommonEntry *Ept01Entry =
          EptGetEptPtEntry(guest_context->stack->processor_data->ept_data,
      ...
      EptCommonEntry *Ept02Pte = EptGetEptPtEntry(
          guest_context->stack->processor_data->EptDat02, fault_pa);
      if (!Ept02Pte || !Ept02Pte->all) {
        // Constructing by nGPA to HPA , EPT0-2 we used it for normal run.
        Ept02Pte = EptpConstructTablesEx(
            guest_context->stack->processor_data->EptDat02->ept_pml4, 4,
            fault_pa, nullptr, nullptr);

        NT_ASSERT(Ept02Pte);
        Ept02Pte->all = Ept01Entry->all;
      }

      NT_ASSERT(Ept02Pte->fields.physial_address ==
      ...
      if (read_failure || write_failure || execute_failure) {
        ...
        HYPERPLATFORM_COMMON_BUG_CHECK(
            ...
            exit_qualification.all, fault_pa, 0);
      }
    } else {
      HYPERPLATFORM_COMMON_BUG_CHECK(
          HyperPlatformBugCheck::kUnexpectedVmEptExit, exit_qualification.all,
          fault_pa, 0);
    }
  } while (0);
...
_Use_decl_annotations_ static void VmmpHandleVmExit(
  ...
  if (kVmmpEnableRecordVmExit) {
    ...
    history.exception_infomation_field = {
        (ULONG32)UtilVmRead(VmcsField::kVmExitIntrInfo)};
  ...
  do {
    ...
    {
      // HYPERPLATFORM_LOG_DEBUG("#1 Almost impossible come here Mode: %x
      // vmcs02_pa: %I64x vmcs_pa: %I64x ", VmxGetVmxMode(vCPU),
...
_Use_decl_annotations_ static void VmmpHandleMonitorTrap(
  ...
  do {
    ...
    EptCommonEntry *entry01 = EptGetEptPtEntry(
        guest_context->stack->processor_data->ept_data,
...
_Use_decl_annotations_ static void VmmpHandleHlt(GuestContext *guest_context) {
  ...
  // UtilVmRead64(VmcsField::kGuestRsp));
  VmmpAdjustGuestInstructionPointer(guest_context);
}
// Interrupt
_Use_decl_annotations_ static void VmmpHandleException(
  ...
  if (interruption_type == InterruptionType::kExternalInterrupt) {
    ...
    HYPERPLATFORM_LOG_DEBUG("GuestRip: %p flags: %x vector: %x",
                            UtilVmRead(VmcsField::kGuestRip), reg, vector);
    VmmpInjectInterruption(interruption_type, vector,
                           exception.fields.error_code_valid, 0);

  } else if (interruption_type == InterruptionType::kHardwareException) {
    if (vector == InterruptionVector::kDebugException) {
      HYPERPLATFORM_LOG_DEBUG(
          "#Tf: %x %I64X %I64X %I64X %I64x", interruption_type,
          UtilVmRead(VmcsField::kGuestRip), UtilVmRead(VmcsField::kGuestRflags),
          UtilVmRead(VmcsField::kGuestCr0), UtilVmRead(VmcsField::kGuestCr4));
      VmmpAdjustGuestInstructionPointer(guest_context);
    }
    // Hardware exception
    if (vector == InterruptionVector::kPageFaultException) {
      ...
      VmmpInjectInterruption(interruption_type, vector,
                             exception.fields.error_code_valid, fault_code.all);
...
_Use_decl_annotations_ static void VmmpHandleMsrAccess(
  ...
  if (read_access) {
    if (transfer_to_vmcs) {
      ...
      switch (msr) {
        case Msr::kIa32VmxEptVpidCap: {
          msr_value.LowPart =
              guest_context->stack->processor_data->VmxEptMsr.LowPart;
          msr_value.HighPart =
              guest_context->stack->processor_data->VmxEptMsr.HighPart;
        } break;
        case Msr::kIa32FeatureControl: {
          msr_value.LowPart =
              guest_context->stack->processor_data->Ia32FeatureMsr.LowPart;
          msr_value.HighPart =
              guest_context->stack->processor_data->Ia32FeatureMsr.HighPart;
          Ia32FeatureControlMsr *da =
              (Ia32FeatureControlMsr *)&msr_value.QuadPart;
          da->fields.lock = true;
          da->fields.enable_vmxon = false;
        } break;
        case Msr::kIa32VmxBasic: {
          msr_value.LowPart =
              guest_context->stack->processor_data->VmxBasicMsr.LowPart;
          msr_value.HighPart =
              guest_context->stack->processor_data->VmxBasicMsr.HighPart;
        } break;
        case Msr::kIa32KernelGsBase:
          msr_value.QuadPart =
              guest_context->stack->processor_data->GuestKernelGsBase.QuadPart;
          break;
        case Msr::kIa32Lstar: {
          HYPERPLATFORM_LOG_INFO(
              "read_access kIa32Lstar && originallstar = "
              "%p\t---------------\r\n",
              guest_context->stack->processor_data
                  ->originallstar);  // originallstar保存原始的msr_lstar
          msr_value.QuadPart =
              (guest_context->stack->processor_data->originallstar != 0)
                  ? guest_context->stack->processor_data->originallstar
                  : UtilReadMsr64(Msr::kIa32Lstar);
          break;
        }
        default: { msr_value.QuadPart = UtilReadMsr64(msr); } break;
      }
    }
    ...
    if (transfer_to_vmcs) {
      ...
      switch (msr) {
        case Msr::kIa32VmxEptVpidCap:
          guest_context->stack->processor_data->VmxEptMsr.QuadPart =
              msr_value.QuadPart;
          break;
        case Msr::kIa32FeatureControl:
          guest_context->stack->processor_data->Ia32FeatureMsr.QuadPart =
              msr_value.QuadPart;
          break;
        case Msr::kIa32VmxBasic:
          guest_context->stack->processor_data->VmxBasicMsr.QuadPart =
              msr_value.QuadPart;
          break;
        case Msr::kIa32KernelGsBase:
          guest_context->stack->processor_data->GuestKernelGsBase.QuadPart =
        ...
        case Msr::kIa32Lstar: {
          HYPERPLATFORM_LOG_INFO(
              "Write_access kIa32Lstar && stack.QuadPart = %p\t &&  "
              "msr_value.QuadPart = %p\t"
              "---------------\r\n",
              guest_context->stack->processor_data->GuestKernelGsBase.QuadPart,
              msr_value.QuadPart);
          if (guest_context->stack->processor_data->originallstar == 0) {
            UtilWriteMsr64(Msr::kIa32Lstar, msr_value.QuadPart);
          }
          break;
        } break;
        default:
...
_Use_decl_annotations_ static void VmmpHandleVmCall(
  ...
  switch (hypercall_number) {
    case HypercallNumber::kHookMsr: {
      guest_context->stack->processor_data->originallstar =
          UtilReadMsr64(Msr::kIa32Lstar);
      UtilWriteMsr64(Msr::kIa32Lstar, guest_context->gp_regs->dx);
      VmmpIndicateSuccessfulVmcall(guest_context);
    } break;
    case HypercallNumber::KUnHookMsr: {
      UtilWriteMsr64(Msr::kIa32Lstar,
                     guest_context->stack->processor_data->originallstar);
      guest_context->stack->processor_data->originallstar = 0;
      VmmpIndicateSuccessfulVmcall(guest_context);
    } break;
    case HypercallNumber::kEpthook: {
      // call shadow page modify attribe
      //  SsSetAttrPageExec(guest_context->stack->processor_data,4);
      VmmpIndicateSuccessfulVmcall(guest_context);
    } break;
    case HypercallNumber::kUnepthook: {
...
_Use_decl_annotations_ static void VmmpHandleEptViolation(
  ...
  if (guest_context->stack->processor_data->EptDat12 &&
      (ULONG64)guest_context->stack->processor_data->EptDat12 !=
          0xFFFFFFFFFFFFFFFF) {
  ...
  EptHandleEptViolation(processor_data->ept_data, 0, is_ranges_of_ept12, NULL,
                        NULL);

  if (is_ranges_of_ept12) {
    EptCommonEntry *entry12 = (EptCommonEntry *)UtilVaFromPa(
        UtilVmRead64(VmcsField::kGuestPhysicalAddress));
    HYPERPLATFORM_LOG_DEBUG_SAFE(0, 0, "EPT1-2 is accesed = %p value= %p ",
                                 UtilVmRead64(VmcsField::kGuestPhysicalAddress),
                                 entry12->all);
...
_Use_decl_annotations_ static void VmmpInjectInterruption(
  ...
  IdtVectoringInformationField idt_vectoring = {
      UtilVmRead(VmcsField::kIdtVectoringInfoField)};
  if (idt_vectoring.fields.valid) {
    if (InterruptionType::kHardwareException !=
            static_cast<InterruptionType>(
                idt_vectoring.fields.interruption_type) ||
        IsBenignException(
            static_cast<InterruptionVector>(idt_vectoring.fields.vector)) ||
        IsBenignException(vector) ||
        (IsContributoryException(
             static_cast<InterruptionVector>(idt_vectoring.fields.vector)) &&
         InterruptionVector::kPageFaultException == vector)) {
    ...
    } else if (InterruptionType::kHardwareException ==
               static_cast<InterruptionType>(
                   idt_vectoring.fields.interruption_type)) {
      if ((IsContributoryException(vector) &&
           IsContributoryException(
               static_cast<InterruptionVector>(idt_vectoring.fields.vector))) ||
          ((IsContributoryException(vector) ||
            InterruptionVector::kPageFaultException == vector) &&
           static_cast<InterruptionVector>(idt_vectoring.fields.vector) ==
        ...
        inject.fields.vector =
            static_cast<ULONG32>(InterruptionVector::kDoubleFaultException);
...
_Use_decl_annotations_ static void VmmpHandleVmx(GuestContext *guest_context) {
  ...
  switch (exit_reason.fields.reason) {
    ...
    */
    ...
    case VmxExitReason::kVmresume: {
      // In VmxVmresumeEmulate will modify Guest Rip so no need to execute
      // anymore after it returns. And should not execute anymore.

      // Vmresume Emulation :
      //- Fill Guest, Host, Control field state in VMCS02
      //- Read GuestRIP from VMCS02 as it is trapped by L2 if Vmresume is
      // trapped by L1
    ...
    case VmxExitReason::kInvept: {
      VmxInveptEmulate(guest_context);
      VMSucceed(&guest_context->flag_reg);
    } break;
    default: {
      UtilInveptGlobal();
  ...
  /***************changed 2017.6.5 by wwq vmresume don't need invoke
