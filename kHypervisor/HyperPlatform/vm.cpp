...
_Use_decl_annotations_ NTSTATUS VmInitialization() {
  ...
  //  Inti Vt Msr hook
  status = UtilForEachProcessor(SHInitMsrHook, nullptr);
  if (!NT_SUCCESS(status)) {
    UtilForEachProcessor(VmpStopVm, nullptr);
    return status;
  }
   ...
   //  Test Vt Ssdt hook
  status = UtilForEachProcessor(EptSshookEntry, shared_data->shared_sh_data);
   if (!NT_SUCCESS(status)) {
    UtilForEachProcessor(VmpStopVm, nullptr);
    return status;
   }

  // Activate installed Ssdt hook
...
_Use_decl_annotations_ static void *VmpBuildMsrBitmap() {
  ...
  // RtlSetBit(
  //          (PRTL_BITMAP)&bitmap_read_low,
  //          (ULONG64)
  //              Msr::kIa32FeatureControl);  // MSR_IA32_FEATURE_CONTROL
  //      RtlSetBit((PRTL_BITMAP)&bitmap_read_low,
  //                  (ULONG64)Msr::kIa32Debugctl);  // MSR_DEBUGCTL
  //      RtlSetBit((PRTL_BITMAP)&bitmap_read_high,
  //                  (ULONG64)Msr::kIa32Lstar -
  //                      0xC0000000);		// MSR_LSTAR
  //									//
  //Ignore IA32_MPERF (000000e7) and
  //									//
  // IA32_APERF (000000e8)
  //      for (ULONG i = (ULONG)Msr::kIa32VmxBasic;
  //           i <= (ULONG)Msr::kIa32VmxVmfunc; i++)
  //        RtlSetBit((PRTL_BITMAP)&bitmap_read_low, i);
  /*
...
_Use_decl_annotations_ static bool VmpSetupVmcs(
  ...
  // Ignore bit 55 of the IA32_VMX_BASIC MSR.
  ...
  vm_procctl2_requested.fields.enable_vpid = true;           // false
