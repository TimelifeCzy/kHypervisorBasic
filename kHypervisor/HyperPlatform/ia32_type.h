...
#define VMX_SECONDARY_PROCESSOR_BASED_VIRTUALIZE_APIC_ACCESSES  (1 <<  0) /* APIC virtualization */
#define VMX_SECONDARY_PROCESSOR_BASED_EPT_ENABLE                (1 <<  1) /* EPT */
#define VMX_SECONDARY_PROCESSOR_BASED_DESCRIPTOR_TABLE_VMEXIT   (1 <<  2) /* Descriptor Table VMEXIT */
#define VMX_SECONDARY_PROCESSOR_BASED_RDTSCP                    (1 <<  3)
#define VMX_SECONDARY_PROCESSOR_BASED_VIRTUALIZE_X2APIC_MODE    (1 <<  4) /* Virtualize X2APIC */
#define VMX_SECONDARY_PROCESSOR_BASED_VPID_ENABLE               (1 <<  5) /* VPID */
#define VMX_SECONDARY_PROCESSOR_BASED_WBINVD_VMEXIT             (1 <<  6) /* WBINVD VMEXIT */
#define VMX_SECONDARY_PROCESSOR_BASED_UNRESTRICTED_GUEST        (1 <<  7) /* Unrestricted Guest */
#define VMX_SECONDARY_PROCESSOR_BASED_VIRTUALIZE_APIC_REGISTERS (1 <<  8)
#define VMX_SECONDARY_PROCESSOR_BASED_VIRTUAL_INT_DELIVERY      (1 <<  9)
#define VMX_SECONDARY_PROCESSOR_BASED_PAUSE_LOOP_VMEXIT         (1 << 10) /* PAUSE loop exiting */
#define VMX_SECONDARY_PROCESSOR_BASED_RDRAND_VMEXIT             (1 << 11)
#define VMX_SECONDARY_PROCESSOR_BASED_INVPCID                   (1 << 12)
#define VMX_SECONDARY_PROCESSOR_BASED_VMFUNC_ENABLE             (1 << 13) /* VM Functions */
#define VMX_SECONDARY_PROCESSOR_BASED_VMCS_SHADOWING            (1 << 14) /* VMCS Shadowing */
#define VMX_SECONDARY_PROCESSOR_BASED_RDSEED_VMEXIT             (1 << 16)
#define VMX_SECONDARY_PROCESSOR_BASED_PML_ENABLE                (1 << 17) /* Page Modification Logging */
#define VMX_SECONDARY_PROCESSOR_BASED_EPT_VIOLATION_EXCEPTION   (1 << 18) /* #VE Exception */
#define VMX_SECONDARY_PROCESSOR_BASED_SUPPRESS_GUEST_VMX_TRACE  (1 << 19) /* Processor Trace */
#define VMX_SECONDARY_PROCESSOR_BASED_XSAVES_XRSTORS            (1 << 20) /* XSAVES */
#define VMX_SECONDARY_PROCESSOR_BASED_PCOMMIT_EXITING           (1 << 21) /* PCOMMIT */
#define VMX_SECONDARY_PROCESSOR_BASED_TSC_SCALING               (1 << 25) /* TSC Scaling */
...
static_assert(sizeof(SegmentSelector) == 8, "Size check");
#include <poppack.h>
/// @copydoc GdtrOrIdtrScaling
enum class VMXAaddressSize {
	k16bit = 0,
	k32bit,
	k64bit,
};
enum class VMXScaling {
	kNoScaling = 0,
	kScaleBy2,
	kScaleBy4,
	kScaleBy8,
...
union MSR_EFER
{
	...
	struct
	{
		ULONG64 sc : 1;			///<[0] SCE (System Call Extensions)
		ULONG64 Reserved1 : 7;	///<[1-7] Reserved
		ULONG64 LME : 1;		///<[8]LME
		ULONG64 Reserved2 : 1;	///<[9]Reserved
		ULONG64 LMA : 1;		///<[10]Long Mode Active
		ULONG64 NXE : 1;		///<[11]No-Execute Active
		...
		ULONG64 Reserved3 : 48;	///<[16-63] Reserve
	}fields;
};
...
enum class VmcsField : unsigned __int32
{
	...
	kPmlAddress = 0x0000200e,
	kPmlAddressHigh = 0x0000200f,
	...
	kPostedInterruptDescAddr = 0x00002016,  /* Posted Interrupts */
	kPostedInterruptDescAddrHigh = 0x00002017,
	kVmFuncCtls = 0x00002018,			    /* VM Functions */
	kVmFuncCtlsHigh = 0x00002019,
	...
	kVmEntryIntrInfoField = 0x00004016,			//Vmentry injection event vol 3b 21-25
	kVmEntryExceptionErrorCode = 0x00004018,		//error code
...
enum class VmxExitReason : unsigned __int16 {
  ...
  kHookMsr = 65,
  KUnHookMsr = 66,
  kEpthook = 67,  // ssdt hook enable
  kUnepthook = 68                 // ssdt unhook disable
};
...
/// See: Format of the VM-Exit Instruction-Information Field as Used for VMCLEAR, VMPTRLD, VMPTRST, and VMXON
union VMInstructionQualificationForClearOrPtrldOrPtrstOrVmxon
{
	ULONG32 all;
	struct
	{
		ULONG32 scalling : 2;				///<[0:1]		
		ULONG32 reserved1 : 5;				///<[2:6]		
		ULONG32 address_size : 3;			///<[7:9]	        16/32/64bits
		ULONG32 reserved2 : 1;				///<[10]		
		ULONG32 reserved3 : 4;				///<[11:14]		
		ULONG32 SegmentRegister : 3;			///<[15:17]		
		ULONG32 IndxeReg : 4;				///<[18:21]		Index register for decode the operand, use this for determin which register is used for passing parameter
		ULONG32 IndexRegInvalid : 1;			///<[22]		Use Index register ?
		ULONG32 BaseReg : 4;				///<[23:26]		Base register for decode the operand
		ULONG32 BaseRegInvalid : 1;			///<[27]		Use Base register ?
		ULONG32 Undefined2 : 4;				///<[28:31]		VMCS Field 
	} fields;
};
static_assert(sizeof(VMInstructionQualificationForClearOrPtrldOrPtrstOrVmxon) == 4, "Size check");

/// See: Format of the VM-Exit Instruction-Information Field as Used for VMREAD and VMWRITE
union VMInstructionQualificationForVmreadOrVmwrite
{
	ULONG32 all;
	struct
	{
		ULONG32 scalling : 2;				/// [0:1]		
		ULONG32 reserved1 : 1;				/// [2]			
		ULONG32 Reg1 : 4;				/// [3:6]	`	if MemOrReg == 1 then it is index 
		ULONG32 address_size : 3;			/// [7:9]		16/32/64bit		
		ULONG32 MemOrReg : 1;				/// [10]		Mem/Reg operand
		ULONG32 Undefined : 4;				/// [11:14]		undefined 
		ULONG32 SegmentRegister : 3;			/// [15:17]		
		ULONG32 IndexReg : 4;				/// [18:21]		Index register for decode the operand, use this for determine which register is used for passing parameter
		ULONG32 IndexRegInvalid : 1;			/// [22]		Use Index register ?
		ULONG32 BaseReg : 4;				/// [23:26]		Base register for decode the operand
		ULONG32 BaseRegInvalid : 1;			/// [27]		Use Base register ?
		ULONG32 Reg2 : 4;				/// [28:31]		VMCS Field 
	} fields;
};
static_assert(sizeof(VMInstructionQualificationForVmreadOrVmwrite) == 4, "Size check");

/// See: Format of the VM-Exit Instruction-Information Field as Used for INVEPT, INVPCID, and INVVPID
union VMInstructionFormatForInveptOrInvpcidOrInvvpid
{
	ULONG32 all;
	struct
	{
		...
		ULONG32 reserved2 : 1;
		ULONG32 reserved3 : 4;
		ULONG32 SegmentRegister : 3;
		ULONG32 GeneralRegister : 4;
		ULONG32 GeneralRegisterValid : 1;
		ULONG32 BaseReg : 4;
		ULONG32 BaseRegInvalid : 1;
		ULONG32 Undefined2 : 4;
	} fields;
};
static_assert(sizeof(VMInstructionFormatForInveptOrInvpcidOrInvvpid) == 4, "Size check");
