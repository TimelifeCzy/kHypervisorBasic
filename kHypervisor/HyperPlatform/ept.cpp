...
NTSTATUS EptpBuildNestedEpt(ULONG_PTR vmcs12_va, EptData *ept_data12,
  ...
  auto Ept12Ptr = reinterpret_cast<EptPointer *>(ExAllocatePoolWithTag(
  ...
  auto Ept02Ptr = reinterpret_cast<EptPointer *>(ExAllocatePoolWithTag(
      NonPagedPoolNx, PAGE_SIZE, kHyperPlatformCommonPoolTag));
  if (!Ept02Ptr) {
  ...
  auto Pml4Entry = reinterpret_cast<EptCommonEntry *>(ExAllocatePoolWithTag(
      NonPagedPoolNx, PAGE_SIZE, kHyperPlatformCommonPoolTag));
  if (!Pml4Entry) {
    ExFreePoolWithTag(Ept12Ptr, kHyperPlatformCommonPoolTag);
...
EptData *EptBuildEptDataByEptp() {
  EptData *EptDataPtr = reinterpret_cast<EptData *>(ExAllocatePoolWithTag(
